...
def test_currency_converter(mock_get: Mock, test_transactions: list[Dict]) -> None:
    """
    Тест функции currency_converter.
    подменяем API_KEY
    подменяем requests.get
    тестовые данные в фикстуре

    :param mock_get:
    :param test_transactions: - (фикстура)
    """
    ...
    # Проверяем транзакцию с номером 1
    result: float = currency_converter(test_transactions, 1)

    assert result == 7500.0
    # Проверяем, что requests.get вызывался с аргументами
